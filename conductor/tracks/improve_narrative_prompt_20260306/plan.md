...
## Phase 2: Narrative Prompt Implementation (TDD) [checkpoint: dc14a21]
Refactor the narrative generation logic to meet the new style and constraint requirements.

- [x] Task: Write failing unit tests in `internal/service/narrative_test.go` to verify the new prompt structure and system instruction content. 4955bf6
- [x] Task: Refactor `NarrativeService.GenerateNarrative` to construct the system instruction (style, character limit, mandatory question) and user prompt. df8a5ee
- [x] Task: Ensure historical context and narrative tone are correctly integrated into the system instructions. df8a5ee
- [x] Task: Verify that tests pass with the new structured prompt logic. df8a5ee
